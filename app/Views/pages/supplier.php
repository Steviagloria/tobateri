    ...
    <div class="col-lg-11 m-auto">
        <div class="card card-light">
            ...
            <div class="card-header d-flex align-items-center mb-2">
                ...
                <a href="<?= base_url('supplier/tambah'); ?>" class="btn btn-light ml-auto"><i class="fas fa-plus"></i></a>
            </div>
            ...
            <div class="card-body">
                <table id="example1" class="table table-bordered table-hover">
                    ...
                    <tbody>
                        <?php foreach ($resultSupplier as $result) : ?>
                            <tr>
                                <td><?= $result['nama_supplier']; ?></td>
                                <td><?= $result['alamat_supplier']; ?></td>
                                <td><?= $result['nohp_supplier']; ?></td>
                                <td>
                                    <a href="<?= base_url('supplier/ubah/' . $result['id_supplier']); ?>" class="text-decoration-none mr-3 text-success" title="Edit">
                                        <i class="fa fa-pen"></i>
                                    </a>
                                    <a href="#" class="text-decoration-none text-danger" onclick="deleteSupplier(this, <?= $result['id_supplier']; ?>, '<?= $result['nama_supplier']; ?>')" title="Hapus">
...
<script>
    ...
    // menampilkan modal
    function deleteSupplier(e, id, nama) {

        eltd = e.parentElement.parentElement;

        ids = id;

        $('#txt_modal').html('Ingin hapus supplier <b>"' + nama + '"</b> ?');
    ...
    $(function() {
        $('#btn_yes').click(function() {
            $.ajax({
                url: "<?= base_url('api/v1/supplier/delete'); ?>",
                data: {
                    id_supplier: ids
                },
                method: "POST",
