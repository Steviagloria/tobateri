    ...
    <div class="col-lg-12 m-auto">
        <div class="card card-light">
            <div class="card-header d-flex align-items-center mb-2">
                ...
                <a href="<?= base_url('transaksi/keluar/add'); ?>" class="btn btn-light ml-auto"><i class="fas fa-plus"></i></a>
            </div>
            ...
            <div class="card-body">
                <table id="example1" class="table table-bordered table-hover">
                    ...
                    <tbody>
                        <?php foreach ($dataTransaksiKeluar as $i => $data) : ?>
                            <tr>
                                <td><?= ++$i; ?></td>
                                <td><?= $data['waktu_transaksi_keluar']; ?></td>
                                ...
                                <td>Rp. <?= number_format(($data['jumlah_barang_keluar'] * $data['harga_barang']), 0, ',', '.'); ?></td>
                                <td>
                                    <a href="<?= base_url('transaksi/keluar/ubah/' . $data['id_transaksi_keluar']); ?>" class="text-decoration-none mr-3 text-success" title="Edit">
                                        <i class="fa fa-pen"></i>
                                    </a>
                                    <a href="#" class="text-decoration-none text-danger" onclick="deleteTransaksiKeluar(this, <?= $data['id_transaksi_keluar']; ?>, '<?= $i; ?>')" title="Hapus">
...
<script>
    ...
    // menampilkan modal
    function deleteTransaksiKeluar(e, id, nama) {
    ...
    $(function() {
        $('#btn_yes').click(function() {
            $.ajax({
                url: "<?= base_url('api/v1/transaksi/keluar/delete'); ?>",
                data: {
                    id_transaksi_keluar: ids
                },
                method: "POST",
